...
//deprecoitumassa as we speak ja uusi NewRecipe korvaamassa lopulta
...
const Create: React.FC<Props> = props => {
  ...
  const [stage, setStage] = useState(true); //reseptinkirjoitusvaiheen siirto, nimi ensin
  const [load, setLoad] = useState(false); //käytetään progressbarin näyttöön
  ...
  useEffect(() => {
    ...
    console.log(storageStage, 'stooragestage');
  ...
  const handlesubmit = async event => {
    ...
    const cloudurl = process.env.REACT_APP_CLOUDURL
      ...
      : ''; //ei itseasiassa poista kaikkea vaan hakee ne uudestaan
    /* await client.resetStore() */ await props.me.refetch(); //hakee vaan me queryn
  ...
  const tokenexpired = () => {
    ...
    /* props.myRecipes.refetch() */ return <Redirect to="/" />;
  };
  ...
  const handlestage = () => {
    const changed = !stage;
    setStage(changed);
    localStorage.setItem('stage', JSON.stringify(changed));
  };
  const stepUp = () => {
    setPhase(phase + 1);
  };
  ...
  return (
    <div>
      <div style={phase1}>
        ...
        {stage ? (
          <input placeholder="Recipe name" onChange={handleRecipename}></input>
        ) : null}
        {stage ? <button onClick={handlestage}>next</button> : null}
        {stage ? null : (
          <form id="recipe" /* onSubmit={handlesubmit} */>
            ...
            ></img>
            <br></br>
            <h3 onClick={handlestage}>{recipeName}</h3>
            {ingredients.map(i => (
              <li key={i.id}>
            ...
            <button onClick={handleIngredientsubmit}>
              add to Ingriedients
            </button>
            <br></br>
